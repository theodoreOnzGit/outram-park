...
\begin{frame}{Your Interface: The FHR Control Panel}
    ...
    \begin{figure}
        ...
        \caption{\footnotesize{\textit{Current State: Stable at 30 MWth}}}
    \end{figure}
\end{frame}
...
\begin{frame}[allowframebreaks]{Advanced Control: Load Following Without Control Rods}
    ...
    \vspace{1em}
    \textbf{Why is this viable for advanced SMRs?}
    \begin{itemize}
        \item \textbf{TRISO fuel:} Much higher thermal margins (\~ 1600°C) compared to traditional UO$_2$ 
			with zircalloy cladding (\~ 800°C, strong degradation via oxidation
			\cite{duriez2019zircaloy})
        \item Temperature excursions during load following stay well within safe limits
        \item Strong negative temperature coefficients enable stable, passive control
    \end{itemize}
\end{frame}

% --- Slide 27b: Turbine Bypass valve --- 
\begin{frame}[allowframebreaks]{Turbine Throttle Control}
	\textbf{PWR Traditional Approach (Submarines and other types):} 
	Turbine bypass valve \cite{zhang2024load}

    \vspace{1em}
    \begin{block}{How It Works}
        When power demand changes, adjust coolant flow rate instead of control rods:
        \begin{itemize}
			\item \textbf{Increase demand:} Decrease flow to turbine, increase flow to bypass valve
			\item \textbf{Decrease demand:} Increase flow to turbine, decrease flow to bypass valve
        \end{itemize}
    \end{block}

    \vspace{1em}
	In essence, exhaust the steam from the steam generator directly to cool 
	rather than generate useful work output. This is rather wasteful, but 
	it works for temporary load following too.
...
\begin{frame}[allowframebreaks]{What You Should Observe}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                \item Higher salt temperature $\rightarrow$ Higher Fuel Temperature
...
\begin{frame}[allowframebreaks]{The Slotin Incident: The Accident}
    ...
    \begin{itemize}
		\item Bright flash of blue light (Not Cherenkov radiation, but ionization)
...
\begin{frame}[allowframebreaks]{High-Temperature Reactors: Better Margins}
    \begin{block}{HTGRs (High-Temperature Gas Reactors) and FHRs}
        These reactors use \textbf{TRISO fuel particles}—ceramic-coated fuel designed to withstand extreme temperatures (\~ 1600°C).
...
% --- Slide 50: Why Feedback Didn't Save Chernobyl ---
\begin{frame}[allowframebreaks]{Why Feedback Didn't Save Chernobyl}
    \textbf{The RBMK had negative fuel temperature feedback—why didn't it stop the accident?}

    \vspace{1em}
    \begin{alertblock}{Positive Feedback Overwhelmed Negative Feedback}
        The positive steam-void coefficient \textit{amplified} the excursion faster than the negative fuel temperature coefficient could compensate.
        
        \vspace{0.5em}
        \textbf{The sequence:}
        \begin{enumerate}
            \item Power increase $\rightarrow$ steam formation $\rightarrow$ \textbf{positive void feedback} (very fast)
            \item Positive feedback $\rightarrow$ more power $\rightarrow$ more steam $\rightarrow$ runaway loop
            \item Fuel temperature rises $\rightarrow$ negative fuel feedback activates (too slow)
            \item By the time negative feedback became significant, the reactor was already physically destroyed
        \end{enumerate}
    \end{alertblock}

    \vspace{1em}
    \textbf{Lesson:} Positive feedback mechanisms can overpower negative ones if they act faster. This is why positive void coefficients are fundamentally unacceptable in reactor design.
\end{frame}

% --- Slide 51: Lessons from Chernobyl ---
...
% --- Slide 52: Comparing the Three Cases ---
